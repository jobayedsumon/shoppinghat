...
@section('content')
    ...
    <div class="card">
    	...
    	<div class="card-body">
    		<div class="row gutters-10">
    			...
    			@php
                    $widg = App\Models\WidgetTwo::first();

                    $lebels = json_decode($widg->widget_two_labels,true);
                    $links = json_decode($widg->widget_two_links,true);
                @endphp
                <div class="col-lg-6">
                    <div class="card shadow-none bg-light">
                        <div class="card-header">
                        ...
                        <div class="card-body">
                            <form action="{{ route('widget_two.update') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
                                    <label>{{ translate('Title') }} ({{ translate('Translatable') }})</label>
                                    <input type="hidden" name="widget_two" value="widget_two">
                                    <input type="text" class="form-control" placeholder="Widget title" name="widget_two" value="{{ $widg->widget_two }}">
                                </div>
                                <div class="card">
                                    <div class="card-header">
                                    ...
                                    <table class="table table-bordered" id="dynamic_field">
                                        @foreach($lebels as $key=> $lebel)
                                          <tr class="table-borderless col-12">
                                              <td class="col-4"><input type="text" placeholder="{{translate('Label')}}" name="widget_two_labels[]" class="form-control name_list" value="{{ $lebel }}" /></td>

                                              <td><input type="url" placeholder="http://" name="widget_two_links[]" class="form-control name_list" value="{{ json_decode($widg->widget_two_links,true)  [$key] }}" /></td>  
                                              <td><button type="button" name="remove" id="'+i+'" class="mt-1 btn btn-icon btn-circle btn-sm btn-soft-danger remove-files">X</button></td>  
                                          </tr>
                                        @endforeach  
                                    </table> 
                                    @endif
                                    <table class="table table-bordered" id="dynamic_field">
                                      ...
                                      <tr class="table-borderless">
                                          {{-- <td class="col-4"><input type="text" placeholder="{{translate('Label')}}" name="widget_two_labels[]" class="form-control name_list" /></td>

                                          <td><input type="text" placeholder="http://" name="widget_two_links[]" class="form-control name_list" /></td>  --}} 
                                          <td class="col-2"><button type="button" name="add" id="add" class="btn btn-soft-secondary btn-sm">Add New</button></td>  
                                      </tr>  
                                    </table>
                                </div>
                                <div class="text-right">
    ...
    <script>
        $(document).ready(function(){      
          ...
          $('#add').click(function(){  
               i++;  
               $('#dynamic_field').append('<tr id="row'+i+'" class="dynamic-added table-borderless">\
                                                <td class="col-4"><input type="text"  name="widget_two_labels[]" placeholder="{{translate('Label')}}" class="form-control name_list" /></td>\
                                                <td><input type="text"  name="widget_two_links[]" placeholder="http://" class="form-control name_list" /></td>\
                                                <td><button type="button" name="remove" id="'+i+'" class="mt-1 btn btn-icon btn-circle btn-sm btn-soft-danger btn_remove">X</button></td>\
                                           </tr>');  
          });  
