 ...
 * ==========================================================
 *
 * Main PHP functions file. © 2017-2021 board.support. All rights reserved.
 *
 */

define('SB_VERSION', '3.3.9');
...
$sb_apps = ['dialogflow', 'slack', 'wordpress', 'tickets', 'woocommerce', 'ump', 'perfex', 'whmcs', 'aecommerce', 'messenger', 'whatsapp', 'armember', 'telegram'];
...
function sb_add_user($settings = [], $settings_extra = [], $hash_password = true) {
    ...
    if (sb_is_agent($settings) && sb_isset(sb_get_active_user(), 'user_type') != 'admin') {
...
function sb_update_user($user_id, $settings, $settings_extra = [], $hash_password = true) {
    ...
    $active_user = sb_get_active_user();
    $query = '';
    if (sb_is_agent($user_type) && sb_isset(sb_get_active_user(), 'user_type') != 'admin') {
        return new SBError('security-error', 'sb_update_user');
    }
    if ($email) {
        $email = sb_db_escape($email);
        $existing_email = sb_db_get('SELECT user_type FROM sb_users WHERE email = "' . $email . '" AND id <> ' . sb_db_escape($user_id));
        if ($existing_email) {
            if (sb_get_setting('duplicate-emails') && !sb_is_agent($existing_email['user_type'])) {
    ...
    if (!empty($settings_extra['phone']) && intval(sb_db_get('SELECT COUNT(*) as count FROM sb_users_data WHERE slug = "phone" AND (value = "' . $settings_extra['phone'][0] . '"' . (strpos($settings_extra['phone'][0], '+') !== false ? (' OR value = "' . str_replace('+', '00', $settings_extra['phone'][0]) . '"') : '') . ') AND user_id <> ' . sb_db_escape($user_id))['count']) > 0 && !sb_get_setting('duplicate-emails')) {
        return new SBValidationError('duplicate-phone');
    }
    if ($profile_image == '') {
...
function sb_update_user_value($user_id, $slug, $value, $name = false) {
    $user_id = sb_db_escape($user_id);
    $user_type = sb_isset(sb_db_get('SELECT user_type FROM sb_users WHERE id = ' . $user_id), 'user_type');
    if (sb_isset(sb_get_active_user(), 'user_type') != 'admin' && (sb_is_agent($user_type) || ($slug == 'user_type' && ($value == 'agent' || $value == 'admin')))) {
...
function sb_is_user_online($user_id) {
    ...
    if (defined('SB_SLACK') && isset($user['user_type']) && sb_is_agent($user['user_type'])) {
        return sb_slack_presence($user_id) == 'online';
    }
    if (sb_pusher_active()) {
...
function sb_update_conversation_department($conversation_id, $department, $message = false) {
    ...
    if ($response) {
        if ($message) {
            ...
            sb_send_agents_notifications($recipients, $message, str_replace('{T}', sb_is_agent() ? sb_get_user_name() : sb_get_setting('bot-name', 'Dialogflow'), sb_('This message has been sent because {T} assigned this conversation to your department.')), $conversation_id, false, false, $department, ['force' => true]);
        }
...
function sb_update_conversation_event($payload_event, $conversation_id, $message_preview = false) {
    ...
    sb_db_query('INSERT INTO sb_messages(user_id, message, creation_time, status_code, attachments, payload, conversation_id) VALUES (' . sb_get_active_user_ID() . ', "", "' . gmdate('Y-m-d H:i:s') . '", 0, "", "' . sb_json_escape(json_encode($payload, JSON_INVALID_UTF8_IGNORE)) . '", ' . sb_db_escape($conversation_id) . ')');
...
function sb_send_agents_notifications($recipients, $message, $bottom_message = false, $conversation_id = false, $attachments = false, $user = false, $department = false, $extra = false) {
    ...
    if (!$is_online) {
        ...
        if ($force || sb_get_multi_setting('sms', 'sms-active-agents')) {
            sb_send_sms($message, $recipient_id ? $recipient_id : $recipients, true, $conversation_id, $attachments);
        }
    }
...
function sb_queue($conversation_id, $department = -1) {
    ...
    if (count($queue) == 0 || $position == 1) {
        ...
        if ($smaller === false) {
            ...
            $smaller = sb_db_get('SELECT id FROM sb_users WHERE user_type = "agent"' . ($query == '' ? '' : ' AND id NOT IN (' . substr($query, 0, -1) . ')') . ($pusher ? '' : ' AND last_activity > "' . gmdate('Y-m-d H:i:s', time() - 30) . '"') . ($department != -1 ? ' AND department = ' . $department : '' ) . ' LIMIT 1');
            if (empty($smaller)) {
                $smaller = false;
            } else {
    ...
    if ($sender_id != -1) {
        ...
        if ($security || $user_id == sb_get_active_user_ID() || !empty($GLOBALS['SB_FORCE_ADMIN'])) {
            ...
            if (!$is_human_takeover && !$queue && ((!$is_sender_agent && sb_get_multi_setting('push-notifications', 'push-notifications-active')) || ($is_sender_agent && sb_get_multi_setting('push-notifications', 'push-notifications-users-active')))) {
                sb_push_notification(sb_get_user_name($sender), $message, $sender['profile_image'], $recipient_id, $conversation_id);
            }
    ...
    if ($message) {
        ...
        switch ($conversation_details['source']) {
            ...
            case 'wa':
                ...
                break;
            case 'tg':
                sb_telegram_send_message($conversation_details['extra'], $message);
                break;
        }
    }
    return false;
...
function sb_set_typing($user_id = false, $conversation_id = false, $source = false) {
    if ($source) {
        if (isset($source[0]) == 'fb') return sb_messenger_set_typing($source[1], $source[2]);
        return false;
    } else return sb_db_query('UPDATE sb_users SET typing = ' . sb_db_escape($conversation_id) . ' WHERE id = ' . sb_db_escape($user_id));
}
...
function sb_init_translations() {
    ...
    if (!empty($SB_LANGUAGE) && $SB_LANGUAGE[0] != 'en') {
        ...
        $is_front_translations = sb_get_setting('front-auto-translations');
        if ($cloud_default_language && ((!$admin && !$is_front_translations) || ($admin && !sb_get_setting('admin-auto-translations')))) $language = $cloud_default_language;
        if ((!$language || $language == 'en') || (!$admin && !isset($_GET['lang']) && !$is_front_translations && !$cloud_default_language)) {
            $SB_TRANSLATIONS = false;
        } else {
            switch ($language) {
                case 'nn':
...
function sb_get_admin_language($user_id = false) {
    $language = strtolower(defined('SB_ADMIN_LANG') ? SB_ADMIN_LANG : (sb_get_user_language(sb_get_setting('admin-auto-translations') ? ($user_id ? $user_id : sb_get_active_user_ID()) : false)));
...
function sb_get_setting_code($array) {
    if (isset($array)) {
        $id = $array['id'];
        $type = $array['type'];
        $content = '<div id="' . $id . '" data-type="' . $type . '"' . (isset($array['setting']) ? ' data-setting="' . $array['setting'] . '"' : '') .' class="sb-setting sb-type-' . $type . '"><div class="sb-setting-content"><h2>' . sb_s($array["title"]) . '</h2><p>' . sb_s($array["content"]) . (isset($array['help']) && (!defined('SB_CLOUD') || defined('SB_CLOUD_DOCS')) ? '<a href="' . (defined('SB_CLOUD_DOCS') ? (SB_CLOUD_DOCS . substr($array['help'], strpos($array['help'], '#'))) : $array['help']) . '" target="_blank" class="sb-icon-help"></a>' : '') . '</p></div><div class="input">';
        switch ($type) {
            case 'color':
            ...
            case 'select':
                ...
                for ($i = 0; $i < count($array['value']); $i++) {
                    $content .= '<option value="' . $array['value'][$i][0] . '">' . sb_s($array['value'][$i][1]) . '</option>';
                }
                $content .= '</select>';
                break;
            case 'checkbox':
            ...
            case 'upload-image':
                ...
                break;
            case 'input-button':
                $content .= '<input type="text"><a class="sb-btn">' . sb_s($array['button-text']) . '</a>';
                break;
            case 'button':
                $content .= '<a class="sb-btn" target="_blank" href="' . $array['button-url'] . '">' . sb_s($array['button-text']) . '</a>';
                break;
            case 'multi-input':
                for ($i = 0; $i < count($array['value']); $i++) {
                    $sub_type = $array['value'][$i]['type'];
                    $content .= '<div id="' . $array['value'][$i]['id'] . '" data-type="' . $sub_type . '" class="multi-input-' . $array['value'][$i]['type'] . '"><label>' . sb_s($array['value'][$i]['title']) . '</label>';
                    switch ($sub_type) {
                        case 'text':
                            $content .= '<input type="text">';
                            break;
                        ...
                        case 'select':
                            ...
                            break;
                        case 'button':
                            $content .= '<a class="sb-btn" target="_blank" href="' . $array['value'][$i]['button-url'] . '">' . sb_s($array['value'][$i]['button-text']) . '</a>';
                    }
                    $content .= '</div>';
                }
                break;
...
function sb_get_block_setting($value) {
    switch ($value)  {
        ...
        case 'subscribe':
            ...
            $message = '[email id="sb-subscribe-form" title="' . sb_rich_value($settings['subscribe-title']) . '" message="' . sb_rich_value($settings['subscribe-msg']) . '" success="' . sb_rich_value($settings['subscribe-msg-success']) . '" placeholder="' . sb_rich_value(sb_isset($settings, 'follow-placeholder', 'Email')) . '" name="' . ($settings_follow['follow-name'] ? 'true' : 'false') . '" last-name="' . ($settings_follow['follow-last-name'] ? 'true' : 'false') . '"]';
...
function sb_pusher_trigger($channel, $event, $data = []) {
    ...
    switch ($event) {
        ...
        case 'update-conversations':
            ...
            break;
        case 'gumroad':
            if ($channel == 'cloud') $security = true;
            break;
    }
    if (defined('SB_CLOUD')) {
                ...
                case 'email':
                                ...
                                if (strlen($value) > 3 && (($is_email && strpos($value, '.')) || (!$is_email && is_numeric(substr($value, 1))))) {
                                    ...
                                    if (!empty($shortcode['success'])) {
                                        ...
                                        switch ($source_name) {
                                            ...
                                            case 'tg':
                                                sb_telegram_send_message($source['chat_id'], $message_new);
                                                break;
                                        }
                    ...
                    if (!$valid) {
                        ...
                        switch ($source_name) {
                            ...
                            case 'tg':
                                sb_telegram_send_message($source['chat_id'], $last_message);
                                break;
                        }
    ...
    if ($dialogflow_active) {
        ...
        if (!$skip_dialogflow && (!sb_get_setting('dialogflow-timetable') || !sb_office_hours())) {
            ...
            for ($i = 0; $i < count($messages); $i++) {
                $messate_text = sb_isset($messages[$i], 'message', '');
                $message_attachments = sb_isset($messages[$i], 'attachments');
                $payload = sb_isset($messages[$i], 'payload');
                if (isset($payload['rich-message'])) $messate_text .= $payload['rich-message'];
                if ($messate_text || isset($messages[$i]['attachments'])) {
                    switch ($source_name) {
                        case 'ig':
                        case 'fb':
                            sb_messenger_send_message($source['psid'], $source['page_id'], $messate_text, $message_attachments, $messages[$i]['id']);
                            break;
                        case 'wa':
                            sb_whatsapp_send_message($source['phone'], $messate_text, $message_attachments);
                        case 'tg':
                            sb_telegram_send_message($source['chat_id'], $messate_text, $message_attachments);
                            break;
                    }
                }
                if ($payload) {
                    ...
                    switch ($source_name) {
                        case 'ig':
                        case 'fb':
                            $args = ['source' => $source_name, 'psid' => $source['psid'], 'page_id' => $source['page_id'], 'attachments' => $attachments];                            break;
                        case 'wa':
                            $args = ['source' => 'wa', 'phone' => $source['phone'], 'attachments' => $attachments];
                        case 'tg':
                            $args = ['source' => 'tg', 'chat_id' => $source['chat_id'], 'attachments' => $attachments];
                            break;
                    }
                    sb_dialogflow_payload($payload, $conversation_id, $message, $args);
                }
        ...
        for ($i = 0; $i < count($bot_messages); $i++) {
            ...
            if ($bot_message) {
                switch ($source_name) {
                    ...
                    case 'tg':
                        sb_telegram_send_message($source['chat_id'], $bot_message['message']);
                }
 ...
 * 5. Load reseller js and css codes
 *
 */
...
function sb_cloud_account() {
    return json_decode(sb_encryption('decrypt', isset($_POST['cloud']) ? $_POST['cloud'] : sb_isset($_GET, 'cloud')), true);
}
