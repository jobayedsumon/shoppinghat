 ...
 * ==========================================================
 *
 * Main Javascript admin file. © 2017-2021 board.support. All rights reserved.
 * 
 */
...
(function ($) {
    ...
    */
    ...
    function dialog(text, type, onConfirm = false, id = '', title = '', scroll = false) {
        ...
        let p = box.find('p');
        box.attr('id', id).setClass('sb-scroll-area', scroll).css('height', scroll ? (parseInt($(window).height()) - 160) + 'px' : '');
                ...
                for (var i = 0; i < this.intents.length; i++) {
                    if (this.intents[i].name == name) {
                        ...
                        if (count > 1) {
                            ...
                            dialog(code, 'info', false, 'intent-preview-box', '', 350);
                        }
                        break;
                    }
                }
            ...
            load: function (language_code) {
                ...
                SBF.ajax({
                    ...
                    for (var i = 0; i < areas.length; i++) {
                        ...
                        code += `<div class="sb-active" data-area="${areas[i]}">`;
    ...
    var SBProfile = {
        ...
        populate: function (user, profile_area) {
            ...
            if (profile_area.hasClass('sb-profile-list-conversation') && SBChat.conversation) {
                ...
                if (!SBF.null(source)) {
                    code += this.profileRow('conversation-source', source == 'fb' ? 'Facebook' : (source == 'wa' ? 'WhatsApp' : (source == 'tm' ? 'Text message' : (source == 'ig' ? 'Instagram' : (source == 'tg' ? 'Telegram' : source)))), sb_('Source'));
                }
            }
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets', 'woocommerce': 'Woocommerce', 'ump': 'Ultimate Membership Pro', 'perfex': 'Perfex', 'whmcs': 'WHMCS', 'aecommerce': 'Active eCommerce', 'messenger': 'Messenger', 'whatsapp': 'WhatsApp', 'armember': 'ARMember', 'telegram': 'Telegram' };
                let updates = false;
        ...
        */
            ...
            if (SBApps.telegram.check(SBChat.conversation)) {
                SBApps.telegram.send(SBChat.conversation.get('extra'), response['message'], response['attachments'], (response) => {
                    if (!('ok' in response) || !response.ok) {
                        dialog('Error. Message not sent to Telegram. Error message: ' + JSON.stringify(response), 'info');
                    }
                });
            }
            if (SB_ADMIN_SETTINGS['smart-reply']) {
        ...
        $(admin).on('click', '.sb-profile-list [data-id="conversation-source"]', function () {
            ...
            if (source == 'whatsapp' && activeUser().getExtra('phone')) {
                ...
                window.open('https://t.me/' + activeUser().getExtra('telegram-id').value);
            }
        });
        ...
        */
        ...
        // Telegram
        $(settings_area).on('click', '#telegram-button .sb-btn', function (e) {
            let token = settings_area.find('#telegram-token input').val().trim();
            e.preventDefault();
